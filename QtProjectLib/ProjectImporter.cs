...
namespace Digia.Qt5ProjectLib
{
    public class ProjectImporter
    {
        ...
        public void ImportProFile(string qtVersion)
        {
            ...
            dialog.Title = SR.GetString("ExportProject_SelectQtProjectToAdd");
            dialog.Filter = "Qt Project files (*.pro)|*.pro|All files (*.*)|*.*";

            if (DialogResult.OK != dialog.ShowDialog())
                return;

            var info = new FileInfo(dialog.FileName);
            if (HelperFunctions.IsSubDirsFile(info.FullName))
            {
                ...
                ImportSolution(info, qtVersion);
            }
            else
            {
                ImportProject(info, qtVersion);
            }
        }
