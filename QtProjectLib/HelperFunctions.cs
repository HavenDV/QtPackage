...
namespace Digia.Qt5ProjectLib
{
    ...
    public class HelperFunctions
    {
        ...
        public static bool IsQtProject(VCProject proj)
        {
            ...
            Project envPro = proj.Object as Project;
        ...
        public static EnvDTE.Project GetSelectedProject( EnvDTE.DTE dte ) {
            try {
                var projects = (System.Array)dte.ActiveSolutionProjects;
                var firstProject = projects.GetValue( 0 );
                if ( firstProject is EnvDTE.Project ) {
                    return (EnvDTE.Project)firstProject;
                }
            }
            catch {}
            return null;
        ...
        /// </summary>
        public static EnvDTE.Project GetSelectedQtProject(EnvDTE.DTE dte)
        {
            // can happen sometimes shortly after starting VS
            if ( dte == null || dte.Solution == null
                || ProjectsInSolution( dte ).Count == 0 )
            {
                return null;
            }

            var project = GetSelectedProject( dte );
            project = project ?? GetSingleProjectInSolution( dte );    
            project = project ?? GetActiveDocumentProject( dte );

            return IsQtProject( project ) ? project : null;
        }
