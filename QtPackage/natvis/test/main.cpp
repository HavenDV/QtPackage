...
int main(int argc, char *argv[])
{
	...
	QStringList stringList{ "one", "two", "three" };
	...
	QList<int> lst_int{ 1, 2, 3 };
    auto lst_int_it = lst_int.begin();
    QListIterator<int> lst_int_it2(lst_int);

	QList<BigStruct> lst_big{ 1, 2, 3 };
    auto lst_big_it = lst_big.begin();
    QListIterator<BigStruct> lst_big_it2(lst_big);

    QQueue<int> queue_int;
    queue_int << 1 << 2 << 3;
    auto queue_int_it = queue_int.begin();

	QQueue<BigStruct> queue_big;
    queue_big << BigStruct(1) << BigStruct(2) << BigStruct(3);
    auto queue_big_it = queue_big.begin();

	QLinkedList<int> lnklst_int{ 1, 2, 3 };
    auto lnklst_int_it = lnklst_int.begin();

	QLinkedList<BigStruct> lnklst_big{ 1, 2, 3 };
    auto lnklst_big_it = lnklst_big.begin();

	QRect rect{ 0, 1, 2, 3 };
	QVector<int> vec_int{ 1, 2, 3 };
	QVector<BigStruct> vec_big{ 1, 2, 3 };
	...
	QHash<QString, int> hash{ { "one", 1 }, { "two", 2 }, { "three", 3 } };

    auto hash_it = hash.begin();
	QHashIterator<QString, int> hash_it2{ hash };

	QMultiHash<QString, int> multihash{
		{ "ones", 1 }, { "ones", 11 }, { "ones", 111 },
		{"tw os", 2 }, { "threes", 3 }, { "threes", 33 }
	};

	QSet<int> set_int{ 1, 2, 3 };
	QSet<BigStruct> set_big{ 1, 2, 3 };
	...
	QPolygon polygon{ QVector<QPoint>{ { 10, 20 }, { 20, 30 } } };
    QPolygonF polygonf{ QVector<QPointF>{ { 10.1, 20.1 },{ 20.1, 30.1 } } };
